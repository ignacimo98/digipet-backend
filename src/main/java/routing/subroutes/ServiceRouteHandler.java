...
public class ServiceRouteHandler extends GenericRouteHandler {
    ...
    public void init() {
        path("/services", () -> {
            post("/availability", map((req, res) -> ServiceHandler.getCaregiverAvailability(model,
                    Integer.parseInt(req.queryParams("idPet")), Integer.parseInt(req.queryParams("idPetOwner")),
                    req.queryParams("startTime"), req.queryParams("endTime"), req.queryParams("location"))));
